...
def get_last_run_date():
    """
    get the last run date from json config file.
    """
...
def save_last_run_date(run_date):
    """
    save the run date to json config file.
    """
    ...
    if args.testmode != "yes":
        ...
        if len(load) != 0:
            try:
                ...
                print(f"web call status: {status}")
                if status == 200:
                    print("success")
                    # update the config file with the max date
                    max_dt = max(out_df['Date'])        
